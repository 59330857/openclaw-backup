#!/bin/bash
# OpenClaw Daily Backup Script (v3.2 — 2026-04-28)
# 适配 4.26+ 存储架构：lcm.db + memory/*.sqlite + tasks/runs.sqlite
...
# 2. SQLite databases (4.26+ storage)
do_rsync "$OPENCLAW_DIR/lcm.db" ""
do_rsync "$OPENCLAW_DIR/memory/main.sqlite" "memory/"
do_rsync "$OPENCLAW_DIR/memory/tts.sqlite" "memory/"
do_rsync "$OPENCLAW_DIR/tasks/runs.sqlite" "tasks/"

# 3. Directories (trailing slash = copy contents)
do_rsync "$OPENCLAW_DIR/agents/" "agents/"
do_rsync "$OPENCLAW_DIR/browser/" "browser/"
do_rsync "$OPENCLAW_DIR/canvas/" "canvas/"
do_rsync "$OPENCLAW_DIR/completions/" "completions/"
do_rsync "$OPENCLAW_DIR/credentials/" "credentials/"
do_rsync "$OPENCLAW_DIR/cron/" "cron/"
do_rsync "$OPENCLAW_DIR/delivery-queue/" "delivery-queue/"
...
do_rsync "$OPENCLAW_DIR/logs/" "logs/"
do_rsync "$OPENCLAW_DIR/media/" "media/"
